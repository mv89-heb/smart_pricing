"""UI/admin extensions for Smart Pricing.
...
def system_health():
    """Authenticated, low-detail readiness check used by the UI."""
    if not session.get("logged_in"):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401

    required = [
        "product", "daily_entry", "price_history", "period_lock", "activity_log",
        "user", "billing_template", "billing_template_item",
    ]
    try:
        ...
        if missing:
            return jsonify({
                "ok": False,
                "error": "מסד הנתונים אינו מעודכן",
                "database": base.db.engine.name,
                "tables_checked": len(required),
                "missing": missing,
            }), 503
...
def browser_price_sync_apply():
    ...
    try:
        for item in updates[:100]:
            ...
            existing = (base.PriceHistory.query
                        .filter_by(product_id=product.id, effective_from=effective_from)
            ...
            if existing:
                ...
                base.db.session.add(base.PriceHistory(
                    ...
                    changed_by=session.get("username", "browser-search"),
                ))
            if effective_from <= base.today_iso():
                product.price = value
...
def _inject_period_report(response):
    """Keep existing UI enhancement injection without touching API responses."""
    if "text/html" not in response.headers.get("Content-Type", ""):
        return response
    try:
        body = response.get_data(as_text=True)
        head_marker = "</head>"
        head_asset = '<link rel="stylesheet" href="/static/responsive-layout.css?v=1">'
        if head_asset not in body and head_marker in body:
            body = body.replace(head_marker, head_asset + head_marker, 1)
        ...
        for script in scripts:
            if script not in body and body_marker in body:
                body = body.replace(body_marker, script + body_marker, 1)
