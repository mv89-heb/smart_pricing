...
def _looks_hashed(value):
    if not value:
        return False
    return value.startswith(("pbkdf2:", "scrypt:", "argon2:", "bcrypt$", "$2a$", "$2b$", "$2y$"))


def _normalise_legacy_password(value):
...
def _migrate_legacy_data():
    ...
    if _table_exists("legacy_v1_users"):
        for r in db.session.execute(text("SELECT username,password,role FROM legacy_v1_users")).mappings():
            ...
            if not db.session.scalar(
                ...
                role = {"admin": "admin", "editor": "editor", "viewer": "viewer"}.get(
                    (r["role"] or "viewer").lower(), "viewer"
                )
                db.session.execute(
                    text("""
                        INSERT INTO users
                            (tenant_id,email,name,password_hash,role,is_active)
                        VALUES (:t,:e,:n,:p,:r,1)
                    """),
                    {
                        "t": tenant.id,
                        "e": email,
                        "n": username,
                        "p": _normalise_legacy_password(r["password"]),
                        "r": role,
                    },
                )
    ...
    if _table_exists("legacy_v1_product"):
        for r in db.session.execute(text("SELECT id,name,price,tag FROM legacy_v1_product ORDER BY id")).mappings():
            pid = db.session.scalar(
                text("SELECT id FROM products WHERE tenant_id=:t AND name=:n"),
                {"t": tenant.id, "n": r["name"]},
            )
            if not pid:
                db.session.execute(
                    text("""
                        INSERT INTO products
                            (tenant_id,sku,name,category,unit,current_price,is_active)
                        VALUES (:t,:sku,:n,:c,:u,:p,1)
                    """),
                    {
                        "t": tenant.id,
                        "sku": f"LEG-{r['id']}",
    ...
    if _table_exists("legacy_v1_price_history"):
        for r in db.session.execute(
            text("SELECT product_id,price,effective_from,changed_at,changed_by FROM legacy_v1_price_history")
        ).mappings():
            ...
            if not db.session.scalar(
                text("SELECT id FROM price_history WHERE product_id=:p AND effective_date=:d"),
                {"p": pid, "d": eff},
            ):
                db.session.execute(
                    text("""
                        INSERT INTO price_history
                            (product_id,price,effective_date,changed_at,changed_by)
                        VALUES (:p,:price,:d,:at,:by)
                    """),
                    {
                        "p": pid,
                        "price": r["price"],
                        "d": eff,
                        "at": r["changed_at"],
                        "by": r["changed_by"] or "מערכת",
                    },
                )

    if _table_exists("legacy_v1_daily_entries"):
        for r in db.session.execute(
            text("SELECT date,product_name,quantity,is_extra,unit_price,note FROM legacy_v1_daily_entries")
        ).mappings():
            pid = db.session.scalar(
                text("SELECT id FROM products WHERE tenant_id=:t AND name=:n"),
                {"t": tenant.id, "n": r["product_name"]},
            )
            if not pid:
                continue
            price = r["unit_price"]
            if price is None:
                price = db.session.scalar(
                    text("SELECT current_price FROM products WHERE id=:p"),
                    {"p": pid},
                )
            db.session.execute(
                text("""
                    INSERT INTO daily_entries
                        (tenant_id,date,product_id,quantity,price_at_time,entry_type,notes,recorded_by)
                    VALUES (:t,:d,:p,:q,:price,:type,:note,:u)
                """),
                {
                    "t": tenant.id,
                    "d": r["date"],
                    ...
                    "q": r["quantity"],
                    "price": price or 0,
                    "type": "extra" if r["is_extra"] else "regular",
                    "note": r["note"],
                    "u": admin_id,
                },
            )
...
def _repair_unhashed_users():
    """Repair legacy/plaintext password values left by an earlier rebuild."""
...
def _ensure_default_tenant_and_admin():
    ...
    if not User.query.filter_by(tenant_id=tenant.id).first():
        db.session.add(
            User(
                tenant_id=tenant.id,
                email="admin@smartpricing.local",
                name="admin",
