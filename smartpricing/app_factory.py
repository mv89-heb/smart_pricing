...
def create_app(config_object=None):
    ...
    uri=app.config["SQLALCHEMY_DATABASE_URI"]; app.config["SQLALCHEMY_ENGINE_OPTIONS"]={"pool_pre_ping":True} if uri.startswith("sqlite") else {"pool_pre_ping":True,"pool_recycle":1800,"pool_size":int(os.getenv("DB_POOL_SIZE","5")),"max_overflow":int(os.getenv("DB_MAX_OVERFLOW","5"))}
    db.init_app(app)
    ...
    def load_user():
        ...
        if request.path.startswith("/static/") or request.endpoint=="pages.login" or request.path in {"/health","/api/system/health"}: return None
