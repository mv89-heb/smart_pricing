...
def test_current_price_is_used_for_current_date_after_price_change():
    ...
    with app.app_context():
        ...
        assert float(row.total_amount) == 60.0
...
def test_editor_can_write():
    client = app.test_client()
    auth(client, role="editor")
    assert client.post("/api/products", json={"name": "מים", "price": 3}, headers=headers()).status_code == 200
    assert client.post("/api/entries", json={"date": "2026-08-05", "product_name": "מים", "quantity": 2}, headers=headers()).status_code == 200
