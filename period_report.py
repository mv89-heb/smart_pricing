...
def _products_to_zero_rows(products, start_date, end_date, existing_product_ids, legacy_added=None):
    """Synthesize zero-quantity report rows for catalog products that have no
    charge entry in the selected period, so newly added products are still
    visible in the report they were created in.

    existing_product_ids: set of Product.id values that already have at least
    one matched entry in the period (those are skipped - no duplicate row).
    legacy_added: optional {normalized_name: timestamp} map used as a
    fallback "added" date for older products created before the
    Product.created_at column existed.
    """
    legacy_added = legacy_added or {}
        ...
        try:
            ...
            for e in entries:
                ...
                if e.is_extra:
                    ...
                    regular_net += total
                existing_product_ids.add(getattr(product, 'id', None))
                rows.append({
                    'id': e.id,
            ...
            zero_rows = _products_to_zero_rows(products, start, end, existing_product_ids, legacy_added)
            for zr in zero_rows:
                product = next((p for p in products if p.id == zr['product_id']), None)
                vat = _vat_for_product(vat_settings, product)
                zr['category'] = vat['category']
                zr['vat_rate'] = vat['vat_rate']
                zr['vat_amount'] = 0.0
                zr['total_with_vat'] = 0.0
