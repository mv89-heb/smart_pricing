(() => {
  ...
  function bar(id,title,body){const b=document.createElement('div');b.id=id;b.className='gf-bar';b.innerHTML=body+`<span class="gf-count" id="${id}-count"></span><button type="button" class="gf-clear" id="${id}-clear">נקה סינון</button>`;return b;}
  ...
  function setupDaily(){
    ...
    const obs=new MutationObserver(apply);obs.observe(tbody,{childList:true});apply();
  }

  function setupPricing(){
    if($('gf-pricing'))return; const body=$('price-list-table-body');if(!body)return; const table=body.closest('table');
    ...
    ['gf-pricing-q','gf-pricing-min','gf-pricing-max'].forEach(id=>$(id).addEventListener('input',apply));$('gf-pricing-scheduled').addEventListener('change',apply);$('gf-pricing-clear').onclick=()=>{['gf-pricing-q','gf-pricing-min','gf-pricing-max','gf-pricing-scheduled'].forEach(id=>$(id).value='');apply();};new MutationObserver(apply).observe(body,{childList:true});apply();
  }
  ...
  function setupDashboard(){
    ...
    ['gf-dashboard-product','gf-dashboard-type'].forEach(id=>$(id).addEventListener('change',apply));['gf-dashboard-min','gf-dashboard-max'].forEach(id=>$(id).addEventListener('input',apply));$('gf-dashboard-clear').onclick=()=>{['gf-dashboard-product','gf-dashboard-type','gf-dashboard-min','gf-dashboard-max'].forEach(id=>$(id).value='');apply();};new MutationObserver(apply).observe(tbody,{childList:true});apply();
  }

  function setupPeriodic(){
    if($('gf-periodic'))return; const body=$('reportBody');if(!body)return; const table=body.closest('table');if(!table)return;const head=table.parentElement.previousElementSibling;const target=head?.parentElement||table.parentElement;
    ...
    ['gf-periodic-product','gf-periodic-type'].forEach(id=>$(id).addEventListener('change',apply));['gf-periodic-min','gf-periodic-max'].forEach(id=>$(id).addEventListener('input',apply));$('gf-periodic-clear').onclick=()=>{['gf-periodic-product','gf-periodic-type','gf-periodic-min','gf-periodic-max'].forEach(id=>$(id).value='');apply();};new MutationObserver(apply).observe(body,{childList:true});apply();
  }

  function init(){style();setupDaily();setupPricing();setupDashboard();setupPeriodic();}
