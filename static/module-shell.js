(() => {
  ...
  function createSidebar(active, user) {
    ...
    el.innerHTML = `
      <div class="module-shell-brand"><div class="module-shell-brand-mark"><i class="fa-solid fa-layer-group"></i></div><div class="module-shell-brand-title">Smart Pricing</div><div class="module-shell-brand-subtitle">ניהול חיובים ומחירים</div></div>
      <nav class="module-shell-nav" aria-label="ניווט ראשי"><div class="module-shell-section">מערכת</div>${linkMarkup('daily')}${linkMarkup('pricing')}${linkMarkup('reports')}${linkMarkup('dashboard')}${linkMarkup('settings')}</nav>
      <div class="module-shell-sidebar-footer"><div class="module-shell-user"><div class="module-shell-avatar"><i class="fa-solid fa-user"></i></div><div><div class="module-shell-user-name">${escapeHtml(user?.username || 'משתמש')}</div><div class="module-shell-user-role">${roleLabel(user?.role)}</div></div></div></div>`;
  ...
  function setupLegacyPage() {
    ...
    const frame = document.createElement('div'); frame.className = 'module-shell-legacy-page';
  ...
  async function init() {
    ...
    document.body.classList.add('module-shell-ready'); applyTheme();
    ...
    if (window.location.pathname === '/') {
      ...
      const legacyFrame = setupLegacyPage(); if (legacyFrame) main.appendChild(legacyFrame);
    }
