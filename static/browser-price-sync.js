(() => {
  ...
  async function periodLockState(effective){
    const month=String(effective||'').slice(0,7);
    if(!/^\d{4}-\d{2}$/.test(month))return {locked:false};
    const r=await api(`/api/periods/${encodeURIComponent(month)}`);
    if(!r?.ok)return {locked:false,unavailable:true};
    return await r.json().catch(()=>({locked:false,unavailable:true}));
  }
  function install(){
  ...
  function open(){
    ...
    $('browser-price-effective').addEventListener('change', refreshLockState);
    refreshLockState();
  }
  function close(){$('browser-price-modal')?.remove()}
  let results=[];
  async function refreshLockState(){
    const input=$('browser-price-effective'); const box=$('browser-price-lock-status'); const applyButton=$('browser-price-apply');
    if(!input||!box)return;
    const state=await periodLockState(input.value);
    box.classList.remove('hidden','border-rose-300','bg-rose-50','text-rose-700','border-amber-300','bg-amber-50','text-amber-700','border-emerald-300','bg-emerald-50','text-emerald-700');
    if(state.locked){box.classList.add('border-rose-300','bg-rose-50','text-rose-700');box.textContent='התקופה שבחרת נעולה. ניתן לצפות ולבדוק מחירים, אך לא ניתן להחיל עדכונים.';if(applyButton)applyButton.disabled=true;return;}
    if(state.unavailable){box.classList.add('border-amber-300','bg-amber-50','text-amber-700');box.textContent='לא ניתן לאמת כרגע את מצב נעילת התקופה. השרת עדיין יאכוף את הנעילה בעת השמירה.';return;}
    box.classList.add('border-emerald-300','bg-emerald-50','text-emerald-700');box.textContent='התקופה פתוחה לעדכון.';
    updateCount();
  }
  async function search(){
    ...
    try{const r=await api('/api/browser-price-sync/search',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({products:list})});const data=r?await r.json():{};if(!r?.ok){toast(data.error||'החיפוש נכשל',true);return;}results=data.results||[];render();status.textContent=`הבדיקה הסתיימה: ${results.length} מתוך ${list.length} מוצרים. מוצגים גם מוצרים שלא נמצא להם מחיר.`;await refreshLockState();}catch(e){toast('שגיאה בבדיקת המחירים',true)}finally{$('browser-price-search').disabled=false;}
  }
  function render(){
  ...
  async function updateCount(){const n=[...document.querySelectorAll('.browser-price-check:checked')].length;$('browser-price-count').textContent=`${n} מוצרים מוכנים לעדכון מתוך ${results.length}`;const button=$('browser-price-apply');if(!button)return;const state=await periodLockState($('browser-price-effective')?.value);button.disabled=n===0||!!state.locked;}
  async function apply(){const effective=$('browser-price-effective').value;if(!effective){toast('בחר תאריך תוקף',true);return;}const state=await periodLockState(effective);if(state.locked){toast('התקופה שבחרת נעולה ולכן לא ניתן לעדכן מחירים.',true);await refreshLockState();return;}const updates=[...document.querySelectorAll('.browser-price-check:checked')].map(c=>{const x=results[Number(c.dataset.index)];return{name:x.name,price:x.price}});if(!updates.length)return;$('browser-price-apply').disabled=true;$('browser-price-status').textContent=`שומר ${updates.length} מחירים...`;const r=await api('/api/browser-price-sync/apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({updates,effective_from:effective})});const data=r?await r.json():{};if(!r?.ok){toast(data.error||'השמירה נכשלה',true);$('browser-price-apply').disabled=false;await refreshLockState();return;}localStorage.setItem('global_price_effective_from',effective);toast(`עודכנו ${data.applied?.length||0} מחירים`);close();if(typeof window.loadProducts==='function')await window.loadProducts();if(typeof window.refreshScheduledPrices==='function')window.refreshScheduledPrices();}
