(() => {
  ...
  async function getProducts(){
    ...
    if(Array.isArray(data))return data.map(p=>({name:p.name,current_price:Number(p.price)||0,tag:p.tag||''}));
    return Object.entries(data||{}).map(([name,price])=>({name,current_price:Number(price)||0}));
  }
  function install(){
