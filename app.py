...
def _run_migrations():
    for sql, table, column in [
        ('ALTER TABLE daily_entry ADD COLUMN unit_price FLOAT', 'daily_entry', 'unit_price'),
        ('ALTER TABLE daily_entry ADD COLUMN note VARCHAR(255)', 'daily_entry', 'note')
    ]:
        try:
...
def entry_json(e):
    return {'id': e.id, 'date': e.date, 'product_name': e.product_name, 'quantity': e.quantity,
            'is_extra': bool(e.is_extra), 'unit_price': e.unit_price, 'note': e.note}

@app.route('/')
def index(): return render_template('index.html')

@app.route('/periodic-report')
def periodic_report(): return render_template('periodic_report.html')
...
@app.route('/logout')
def logout():
    log_activity('LOGOUT', 'התנתקות מהמערכת'); session.clear(); return redirect(url_for('login'))
...
def add_product():
    ...
    try:
        ...
        if product:
            product.price = price; log_activity('UPDATE_PRICE', f'מוצר: {name}, מחיר: {price}')
        else:
            db.session.add(Product(name=name, price=price)); log_activity('NEW_PRODUCT', f'מוצר חדש: {name}')
...
def add_entry():
    ...
    try:
        ...
        product = Product.query.filter_by(name=product_name).first(); current_price = product.price if product else 0
        entry = DailyEntry.query.filter_by(date=date, product_name=product_name, is_extra=is_extra).first()
        if entry and not note:
            entry.quantity += quantity; entry.unit_price = current_price
        else:
            db.session.add(DailyEntry(date=date, product_name=product_name, quantity=quantity, is_extra=is_extra, unit_price=current_price, note=note or None))
        db.session.commit(); return jsonify({"success": True})
...
def delete_entry(entry_id):
    ...
    try:
        entry = db.session.get(DailyEntry, entry_id)
        if entry:
            details = f'חיוב #{entry.id}: {entry.product_name}, {entry.date}'
            db.session.delete(entry); db.session.commit(); log_activity('DELETE_ENTRY', details)
        return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def clear_date_entries(date):
    if is_viewer(): return jsonify({"success": False, "error": "אין הרשאות"}), 403
    try:
        DailyEntry.query.filter_by(date=date).delete(); db.session.commit(); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def reset_season():
    ...
    try:
        DailyEntry.query.delete(); db.session.commit(); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500

@app.route('/api/report/month/<year_month>', methods=['GET'])
def get_monthly_report(year_month):
    if len(year_month) != 7 or year_month[4] != '-' or not year_month[:4].isdigit() or not year_month[5:].isdigit():
        return jsonify({"error": "חודש לא תקין"}), 400
    entries = DailyEntry.query.filter(DailyEntry.date.startswith(year_month)).order_by(DailyEntry.date.asc(), DailyEntry.id.asc()).all()
...
def get_period_report():
    ...
    if not start or not end or len(start) != 10 or len(end) != 10 or start > end:
    ...
    rows = [entry_json(e) for e in entries]
    regular = extra = 0.0
    product_summary = {}
    day_summary = {}
    for e in entries:
        total = float(e.quantity or 0) * float(e.unit_price or 0)
        bucket = product_summary.setdefault(e.product_name, {'quantity': 0.0, 'total': 0.0})
        bucket['quantity'] += float(e.quantity or 0); bucket['total'] += total
        day = day_summary.setdefault(e.date, {'regular': 0.0, 'extra': 0.0, 'total': 0.0})
        if e.is_extra: extra += total; day['extra'] += total
        else: regular += total; day['regular'] += total
        day['total'] += total
    return jsonify({
        'from': start, 'to': end, 'entries': rows,
        'summary': {'regular_total': regular, 'extra_total': extra, 'grand_total': regular + extra,
                    'days_count': len(day_summary), 'average_day': (regular + extra) / len(day_summary) if day_summary else 0},
        'product_summary': product_summary, 'day_summary': day_summary
    })
