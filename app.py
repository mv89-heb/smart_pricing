...
def price_history_json(product):
    ...
    return [{"id": r.id, "price": float(r.price), "effective_from": r.effective_from, "changed_at": r.changed_at.isoformat(),
             "changed_by": r.changed_by, "scheduled": bool(r.effective_from and r.effective_from > today)} for r in rows]
...
def update_product(name):
    ...
    new_name = (data.get("name") or name).strip(); tag = (data.get("tag") if "tag" in data else p.tag or "").strip()[:80]
    try: new_price = money(data.get("price", p.price))
    except Exception: return jsonify({"success": False, "error": "מחיר לא תקין"}), 400
    effective = (data.get("effective_from") or today_iso()).strip()
    if not new_name or new_price < 0 or not valid_date(effective): return jsonify({"success": False, "error": "נתונים שגויים"}), 400
    if new_name != name and Product.query.filter_by(name=new_name).first(): return jsonify({"success": False, "error": "מוצר קיים"}), 409
    try:
        old_name = name
        if new_name != name:
            DailyEntry.query.filter_by(product_name=name).update({DailyEntry.product_name: new_name}); BillingTemplateItem.query.filter_by(product_name=name).update({BillingTemplateItem.product_name: new_name}); p.name = new_name
        p.tag = tag or None
        existing = PriceHistory.query.filter_by(product_id=p.id, effective_from=effective).order_by(PriceHistory.id.desc()).first()
        if existing:
            existing.price = new_price; existing.changed_at = datetime.utcnow(); existing.changed_by = session.get("username", "מערכת")
        else:
            db.session.add(PriceHistory(product_id=p.id, price=new_price, effective_from=effective, changed_by=session.get("username", "מערכת")))
        if effective <= today_iso(): p.price = new_price
        db.session.commit(); log_activity("UPDATE_PRICE", f"מוצר: {old_name} -> {new_name}; מחיר {new_price}; תקף מ-{effective}"); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def product_history(name):
    ...
    return jsonify(price_history_json(p))

@app.route("/api/products/<path:name>/scheduled", methods=["DELETE"])
def cancel_scheduled_price(name):
    denied = write_access()
    if denied: return denied
    p = Product.query.filter_by(name=name).first()
    if not p: return jsonify({"success": False, "error": "לא נמצא"}), 404
    try:
        rows = PriceHistory.query.filter(PriceHistory.product_id == p.id, PriceHistory.effective_from > today_iso()).all()
        for row in rows: db.session.delete(row)
        db.session.commit(); log_activity("CANCEL_SCHEDULED_PRICE", f"ביטול מחיר עתידי: {name}"); return jsonify({"success": True, "cancelled": len(rows)})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def add_entry():
    denied = write_access()
    if denied: return denied
    data = request.get_json(silent=True) or {}; entry_date = data.get("date"); product_name = (data.get("product_name") or "").strip(); note = (data.get("note") or "").strip()[:255]
    if not valid_date(entry_date) or not product_name: return jsonify({"success": False, "error": "נתונים שגויים"}), 400
    if period_locked(entry_date): return jsonify({"success": False, "error": "התקופה נעולה. נדרש מנהל כדי לשנות חיובים."}), 423
    ...
    if not product: return jsonify({"success": False, "error": "המוצר לא נמצא במחירון"}), 404
    current_price = price_for_date(product, entry_date); extra = bool(data.get("is_extra", False))
    try:
        existing = DailyEntry.query.filter_by(date=entry_date, product_name=product_name, is_extra=extra).first()
        if existing and not note and money(existing.unit_price) == current_price:
            ...
            db.session.add(DailyEntry(date=entry_date, product_name=product_name, quantity=quantity, is_extra=extra, unit_price=current_price, total_amount=(money(quantity) * current_price).quantize(Decimal("0.01")), note=note or None))
        db.session.commit(); log_activity("ADD_ENTRY", f"{entry_date} | {product_name} | {quantity} | {current_price}"); return jsonify({"success": True, "unit_price": float(current_price)})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def update_entry(entry_id):
    ...
    try:
        ...
        db.session.rollback(); return jsonify({"success": False, "error": "נתונים שגויים"}), 400
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def delete_entry(entry_id):
    ...
    try:
        db.session.delete(e); db.session.commit(); log_activity("DELETE_ENTRY", f"חיוב נמחק: {details}"); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def clear_date_entries(date):
    ...
    try:
        DailyEntry.query.filter_by(date=date).delete(); db.session.commit(); log_activity("CLEAR_DAY", f"ניקוי יום: {date}"); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def reset_season():
    ...
    if denied: return denied
    try:
        DailyEntry.query.delete(); db.session.commit(); log_activity("RESET_SEASON", "מחיקת כל החיובים"); return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
...
def delete_user(user_id):
    ...
    try:
        ...
        if u and u.username != session.get("username"):
            name = u.username; db.session.delete(u); db.session.commit(); log_activity("USER_DELETE", f"משתמש: {name}")
        return jsonify({"success": True})
    except SQLAlchemyError:
        db.session.rollback(); return jsonify({"success": False, "error": "שגיאת שרת"}), 500
