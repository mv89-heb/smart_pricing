...
def login():
    ...
    if user and valid:
        session['logged_in']=True; session['username']=user.username; session['role']=user.role; log_activity('LOGIN','התחברות למערכת')
...
def get_period_report():
    ...
    regular=extra=0.0; product_summary={}; day_summary={}
    for e in entries:
        total=float(e.quantity or 0)*float(e.unit_price or 0); p=product_summary.setdefault(e.product_name,{'quantity':0.0,'total':0.0}); p['quantity']+=float(e.quantity or 0); p['total']+=total
